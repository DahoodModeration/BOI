...
async def on_ready():
    ...
    while True:
        ...
        activity = discord.Activity(type=discord.ActivityType.watching, name=f'Eero#7793')
...
async def bronze(ctx):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed=discord.Embed(title="Free Bronze",color=100080, description=f"**set .gg/wb2zNCgcxA status, you will automatically get a rank Bronze**")
    await ctx.send(embed=embed)
...
async def on_member_update(before, after):
    role_id = 1102265287597895730
    role = get(before.guild.roles, id=role_id)
    if '.gg/wb2zNCgcxA' in str(before.activities):
      if '.gg/wb2zNCgcxA' in str(after.activities):
        pass
      else:
        await after.remove_roles(role)
        channel = bot.get_channel(1102265287597895730)
...
@bot.command()
async def clear(ctx):
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/1102269874539073536/UqyCgAZHrXRWCWqyCZ806Y1L5hCgxPwmgMLowzvuZr-8uSJ9tkBZwdHv0dorsWhsH3LI')
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}clear**'), color='9B59B6')
...
async def tfollow(ctx, arg):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}tfollow -> {arg}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel =json_object['bot_config']["twitch_channel"]
    ...
    if ctx.channel.id == int(genchannel) or ctx.channel.id == int(genchannel2):
        ...
        for role_name in role_config:
            ...
            if role_id in ctx.author.roles:
                ...
                filefile = open('tokens_follow.txt')
                num_lines = sum(1 for line in filefile)
                filefile.close()
                
                filefile = open('tokens_follow.txt', 'r')
                ...
                def start_follow():
                    ...
                    for i in range(caunt_to_follow):
                        ...
                        try:
                            tokens = random.choice(open("tokens_follow.txt", "r" ).read().splitlines())
                            ...
                            try:
                                if response.json()[0]['data']['followUser']['error']:
                                    with open("tokens_follow.txt", "r") as f:
                                        lines = f.readlines()
                                    with open("tokens_follow.txt", "w") as f:
                                        for line in lines:
                                            if line.strip("\n") != tokens:
                                                f.write(line)
                                                f.close()
...
async def tspam(ctx, arg1, *, args):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/1102269874539073536/UqyCgAZHrXRWCWqyCZ806Y1L5hCgxPwmgMLowzvuZr-8uSJ9tkBZwdHv0dorsWhsH3LI')
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}tspam -> {arg1} Message: {args}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel = json_object['bot_config']["twitch_channel"]
    genchannel2 = json_object['bot_config']["twitch_channel_2"] 
...
async def treport(ctx, arg):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/1102269874539073536/UqyCgAZHrXRWCWqyCZ806Y1L5hCgxPwmgMLowzvuZr-8uSJ9tkBZwdHv0dorsWhsH3LI')
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}treport -> {arg}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel = json_object['bot_config']["twitch_channel"]
    genchannel2 = json_object['bot_config']["twitch_channel_2"]
...
async def tfriend(ctx, arg):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}tfriend -> {arg}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel = json_object['bot_config']["twitch_channel"]
    genchannel2 = json_object['bot_config']["twitch_channel_2"]
...
async def ttroll(ctx, arg1):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/958071838276190318/hyiJYup37H4UL6TJZI9-ohILVsYab2NdhDWhqFKz_n1GzIQ2L6nVsLsBqb-FxXes42kd')
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}ttroll -> {arg1}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel = json_object['bot_config']["twitch_channel"]
    genchannel2 = json_object['bot_config']["twitch_channel_2"]  
...
async def traid(ctx, arg1):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/958071838276190318/hyiJYup37H4UL6TJZI9-ohILVsYab2NdhDWhqFKz_n1GzIQ2L6nVsLsBqb-FxXes42kd')
    embed = DiscordEmbed(title='EeroYt', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}traid -> {arg1}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel = json_object['bot_config']["twitch_channel"]
    genchannel2 = json_object['bot_config']["twitch_channel_2"]  
